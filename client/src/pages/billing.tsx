...
export default function BillingPage() {
  ...
  const getPlanIcon = (planName: string) => {
    switch (planName.toLowerCase()) {
      case 'basic': return <Star className="h-8 w-8 text-blue-600" />;
      case 'pro': return <Crown className="h-8 w-8 text-purple-600" />;
      case 'enterprise': return <Building className="h-8 w-8 text-orange-600" />;
    ...
    if (planName.toLowerCase() === 'pro') {
      return (
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
          <Badge className="bg-purple-600 text-white px-6 py-1">Most Popular</Badge>
        </div>
      );
    }
    return null;
  ...
  if (plansLoading || subscriptionLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <UnifiedHeader showBackButton={true} showAuthButtons={false} />
