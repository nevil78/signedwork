...
export class DatabaseStorage implements IStorage {
  ...
  async getCompanyEmployeesPaginated(companyId: string, options: {
    ...
    if (options.status && options.status !== 'all') {
      whereConditions = and(
        whereConditions,
        eq(companyEmployees.status, options.status)
      );
    }
    ...
    if (options.department && options.department !== 'all') {
      whereConditions = and(
        whereConditions,
        eq(companyEmployees.department, options.department)
      );
    }
    ...
    if (options.search) {
      ...
      whereConditions = and(
        whereConditions,
        or(
          sql`LOWER(CONCAT(${employees.firstName}, ' ', ${employees.lastName})) LIKE ${searchTerm}`,
          sql`LOWER(${employees.email}) LIKE ${searchTerm}`,
          sql`LOWER(${employees.employeeId}) LIKE ${searchTerm}`,
          sql`LOWER(${companyEmployees.position}) LIKE ${searchTerm}`,
          sql`LOWER(${companyEmployees.department}) LIKE ${searchTerm}`
        )
      );
    }
    ...
    const sortColumn = sortBy === 'employeeName' 
      ? sql`CONCAT(${employees.firstName}, ' ', ${employees.lastName})`
      : sortBy === 'employeeEmail' 
      ? employees.email
      : sortBy === 'position'
      ? companyEmployees.position
      : sortBy === 'department'
      ? companyEmployees.department
      : sortBy === 'status'
      ? companyEmployees.status
      : companyEmployees.joinedAt;
